...
<head>
    ...
    <title>Artigos sobre Sistema PDV | <?= eArt($siteName) ?></title>
    ...
    <link rel="canonical" href="<?= eArt($canonical) ?>">

    <meta property="og:type" content="website">
    <meta property="og:title" content="Artigos sobre Sistema PDV | <?= eArt($siteName) ?>">
    ...
    <meta property="og:url" content="<?= eArt($canonical) ?>">
    <meta property="og:locale" content="pt_BR">
    <meta property="og:site_name" content="<?= eArt($siteName) ?>">
    ...
    <script type="application/ld+json">
    {
      ...
      "publisher": {
        "@type": "Organization",
        "name": "Balcão PDV",
        "url": "<?= eArt(rtrim(APP_URL ?? '', '/')) ?>"
      },
      "blogPost": [
        <?php foreach ($artigos as $i => $a): ?>
        {
          "@type": "BlogPosting",
          "headline": "<?= eArt($a['titulo']) ?>",
          "description": "<?= eArt($a['descricao']) ?>",
          "datePublished": "<?= eArt($a['data']) ?>",
          "url": "<?= eArt($canonical . $a['slug']) ?>"
        }<?= $i < count($artigos) - 1 ? ',' : '' ?>
        <?php endforeach; ?>
      ]
    }
...
<nav class="navbar navbar-expand-lg navbar-light bg-white shadow-sm">
    <div class="container">
        <a class="navbar-brand" href="<?= eArt(rtrim(APP_URL ?? '', '/')) ?>/">
        ...
        <div class="collapse navbar-collapse" id="nav">
            <ul class="navbar-nav ms-auto align-items-lg-center">
                <li class="nav-item"><a class="nav-link" href="<?= eArt(rtrim(APP_URL ?? '', '/')) ?>/">Início</a></li>
                <li class="nav-item"><a class="nav-link active fw-bold" href="<?= eArt($canonical) ?>">Artigos</a></li>
                <li class="nav-item"><a class="nav-link" href="<?= eArt(rtrim(APP_URL ?? '', '/')) ?>/auth/register.php">Cadastrar</a></li>
                <li class="nav-item"><a class="btn btn-primary ms-lg-2" href="<?= eArt(rtrim(APP_URL ?? '', '/')) ?>/auth/login.php">Entrar</a></li>
            </ul>
        </div>
    </div>
</nav>
...
<header class="hero-blog">
    <div class="container">
        ...
        <nav class="breadcrumb-blog" aria-label="breadcrumb">
            <a href="<?= eArt(rtrim(APP_URL ?? '', '/')) ?>/">Início</a> <i class="fas fa-chevron-right mx-2" style="font-size:0.7rem;"></i>
            <span>Artigos</span>
        </nav>
    </div>
</header>
    ...
    <div class="container">
        <div class="row g-4">
            ...
            <div class="col-md-6 col-lg-4">
                <a href="<?= eArt($canonical . $a['slug']) ?>" class="article-card" aria-label="Ler artigo: <?= eArt($a['titulo']) ?>">
                    <div class="article-icon">
                        <i class="fas <?= eArt($a['icone']) ?>"></i>
                    </div>
                    <div class="article-body">
                        <span class="article-category"><?= eArt($a['categoria']) ?></span>
                        <h2><?= eArt($a['titulo']) ?></h2>
                        <p><?= eArt($a['descricao']) ?></p>
...
<footer class="footer-blog">
    <div class="container">
        ...
        <p class="mb-0"><a href="<?= eArt(rtrim(APP_URL ?? '', '/')) ?>/">Voltar ao site</a></p>
    </div>
</footer>
